<?php
/****************************************************************************************
* LiveZilla langpl.php by lukas
* 
* Copyright 2010 LiveZilla GmbH
* All rights reserved.
* LiveZilla is a registered trademark.
*
* Please report errors here: http://www.livezilla.net/forum/
* DO NOT REMOVE/ALTER <!--placeholders-->
*
***************************************************************************************/ 

$LZLANG["client_queue_message"] = "Liczba oczekujących klientów <!--queue_position-->. Szacowany czas oczekiwania: <!--queue_waiting_time--> minut(y).";
$LZLANG["client_queue_next_operator"] = "Kolejny konsultant jest dostępny.";
$LZLANG["client_chat_invitation"] = "Zaproszenie do rozmowy";
$LZLANG["client_system"] = "System";
$LZLANG["client_required_field"] = "Wymagane pola";
$LZLANG["client_int_is_connected"] = "Prosimy o chwilę cierpliwości, za chwilę zostaniesz połączony z jednym z naszych konsultantów.";
$LZLANG["client_ints_are_busy"] = "Obecnie wszyscy nasi konsultanci prowadzą rozmowy, prosimy o chwilę cierpliwości.";
$LZLANG["client_int_left"] = "Konsultant zakończył właśnie rozmowę. Jeżeli masz dodatkowe pytania, prosimy o pozostawienie wiadomości:";
$LZLANG["client_int_declined"] = "Wszyscy nasi konsultanci są obecnie zajęci. Przepraszamy, ale Twoje połączenie nie może być zrealizowane w tej chwili. Prosimy spróbować ponownie lub pozostawić wiadomość:";
$LZLANG["client_no_intern_users"] = "Obecnie żaden z konsultantów w tym dziale nie jest dostępny. Prosimy spróbować ponownie za chwilę,lub wybrać inny dział. Możesz także zostawić nam wiadomość:";
$LZLANG["client_no_intern_users_short"] = "Obecnie żaden z konsultantów w tym dziale nie jest dostępny. Prosimy spróbować ponownie za chwilę,lub wybrać inny dział.";
$LZLANG["client_thank_you"] = "Dziękujemy!";
$LZLANG["client_con_broken"] = "Wystąpił błąd połączenia z serwerem: próba ponownego połączenia, prosimy o chwilę cierpliwości ...";
$LZLANG["client_guide_request"] = "Nasz Konsultant spróbuje teraz przekierować cię na stronę:";
$LZLANG["client_still_waiting_int"] = "Jeżeli uważasz, że czas oczekiwania jest zbyt długi-pozostaw nam wiadomoć, skontaktujemy się z tobą poprzez adres e-mail:";
$LZLANG["client_request"] = "Oczekiwanie";
$LZLANG["client_loading"] = "Ładowanie";
$LZLANG["client_forwarding"] = "Trwa przeniesienie do innego konsultanta, prosimy o chwilę cierpliwości.";
$LZLANG["client_vcard"] = "Wizytówka";
$LZLANG["client_guest"] = "Gość";
$LZLANG["client_send_file"] = "Wyślij plik";
$LZLANG["client_representative_is_typing"] = "Konsultant pisze wiadomość ...";
$LZLANG["client_save_visitcard"] = "Pobierz Wizytówkę";
$LZLANG["client_please_enter_name_and_email"] = "Wpisz swoje imię oraz adres e-mail";
$LZLANG["client_start_system"] = "Trwa uruchamianie.";
$LZLANG["client_your_email"] = "Adres email";
$LZLANG["client_your_company"] = "Firma";
$LZLANG["client_your_name"] = "Nazwa użytkownika";
$LZLANG["client_your_message"] = "Twoja wiadomość";
$LZLANG["client_group"] = "Dział";
$LZLANG["client_send_message"] = "Wyślij wiadomość";
$LZLANG["client_error_groups"] = "Usługa nie jest obecnie aktywna.Prosimy o kontakt e-mail.";
$LZLANG["client_error_deactivated"] = "Usługa nie jest obecnie uaktywniona. Prosimy o kontakt e-mail.";
$LZLANG["client_error_unavailable"] = "Usługa nie jest obecnie dostępna. Przepraszamy za problemy.";
$LZLANG["client_leave_message_short"] = "Zostaw wiadomość";
$LZLANG["client_select_valid_group"] = "Proszę wybrać jeden z dostępnych Działów.";
$LZLANG["client_important_hint"] = "Ważna wskazówka";
$LZLANG["client_print"] = "Drukuj";
$LZLANG["client_close_window"] = "Zamknij okno";
$LZLANG["client_rate_representative"] = "Oceń jakośc naszej obsługi";
$LZLANG["client_bookmark"] = "Dodaj do Ulubionych";
$LZLANG["client_insert_smiley"] = "Wprowadź emotkę";
$LZLANG["client_goto_login"] = "Idź do okna logowania";
$LZLANG["client_switch_sounds"] = "Dźwięki włącz/wyłącz";
$LZLANG["client_intern_arrives"] = "<!--intern_name--> jest obecnie dostępny.";
$LZLANG["client_start_chat"] = "Rozpocznij czat";
$LZLANG["client_new_messages"] = "Nowe wiadomości";
$LZLANG["client_message_too_long"] = "Twoja wiadomość jest za długa. Prosimy o jej skrócenie.";
$LZLANG["client_message_received"] = "Wiadomość została wysłana, odpowiemy na nią w możliwie najkrótszym czasie.";
$LZLANG["client_message_flood"] = "Ilość wysłanych przez Ciebie wiadomości przekroczyła dozwolony limit. Proszę spróbować ponownie później.";
$LZLANG["client_really_close"] = "Czy na pewno chcesz zakończyć rozmowę?";
$LZLANG["client_file_request_rejected"] = "Przepraszamy, konsultant nie zaakceptował Twojego zgłoszenia.";
$LZLANG["client_file_upload_oversized"] = "Twój plik został odrzucony przez nasz Serwer z powodu przekroczenia limitu!";
$LZLANG["client_file_upload_provided"] = "Twój plik został pomyślnie przesłany do Konsultanta.";
$LZLANG["client_file_upload_requesting"] = "Oczekiwanie na uprawnienia do przesłania pliku ...";
$LZLANG["client_file_upload_send_file"] = "Aby wysłać wybrany plik, kliknij na WYŚLIJ";
$LZLANG["client_file_upload_select_file"] = "Proszę wybrać plik do wysłania.";
$LZLANG["client_wait_for_representative"] = "Prosimy o chwilę cierpliwości ...";
$LZLANG["client_no_representative"] = "Konsultant zakończył rozmowę.";
$LZLANG["client_transmitting_file"] = "Przesyłanie pliku ...";
$LZLANG["client_please_rate"] = "Prosimy o ocenę naszej obsługi";
$LZLANG["client_rate_reason"] = "Jeżeli masz zastrzeżenia do jakości naszej obsługi, prosimy o kontakt e-mail";
$LZLANG["client_rate_qualification"] = "Kwalifikacje";
$LZLANG["client_rate_politeness"] = "Uprzejmość";
$LZLANG["client_rate_success"] = "Dziękujemy za ocenę i zapraszamy do skorzystania z naszej oferty.";
$LZLANG["client_rate_max"] = "Dzienny limit jest ograniczony do trzech ocen.";
$LZLANG["client_file"] = "Plik";
$LZLANG["client_welcome"] = "Witamy";
$LZLANG["client_send"] = "Wyślij";
$LZLANG["client_abort"] = "Przerwij";
$LZLANG["index_write_access"] = "Dostęp do zapisu";
$LZLANG["index_no_write_access"] = "Przykro nam, nie posiadasz uprawnień do zapisu";
$LZLANG["index_phpversion_needed"] = "Wymagana jest wersja <!--version--> lub wyższa.";
$LZLANG["index_solve"] = "Przeczytaj jak rozwiązać ten problem ..";
$LZLANG["index_adding_server"] = "Dodaj Profil tego Serwera LiveZilla";
$LZLANG["index_adding_server_hint"] = "Musisz zainstalować LiveZilla na twoim komputerze";
$LZLANG["index_close"] = "Zamknij";
$LZLANG["index_not_available"] = "Niedostępne";
$LZLANG["index_addserver"] = "Dodaj Serwer";
$LZLANG["index_description_install"] = "Możesz zainstalować LiveZilla bezpośrednio z tego Serwera. Bezpośrednio po pobraniu i instalacji będziesz mógł połaczyć się z tym Serwerem LiveZilla lub stworzyć swój własny bezpłatny Serwer LiveZilla zgodnie ze swoimi potrzebami.";
$LZLANG["index_description_profile"] = "Klikając tutaj możesz pobrać Profil tego Serwera LiveZilla. Profil Serwera zawiera wszystkie informacje potrzebne do połączenia z tym Serwerem.";
$LZLANG["index_description_demochat"] = "Rozpocznij czat demo, aby spróbować rozmowy z innymi użytkownikami którzy są aktualnie zalogowani. W ten sposób sprawdzisz możliwości aplikacji.";
?>